...
class Tutorial(st.Estados_Juego):
    def __init__(self):
        ...
        self.arrow_right = BotonCarta(self.fondo.getX() + 330, self.fondo.getY() + 190, 30, 30, "right.png", False, False)
    ...
    def createBG(self):
        self.bg_array = []
        for i in range(1,6):
            temp = pygame.image.load(st.current_dir() + "/data/other/{}.png".format(str(i))).convert_alpha()
    ...
    def render(self, clock, screen, p):
        ...
        while not self.st_done: 
            ...
            question_text = tt.render_text("S", self.strings[self.i], self.background_color)
            screen.blit(self.fondo.getImg(),
                        (self.fondo.getX(), self.fondo.getY()))
            screen.blit(question_text, (self.fondo.getRekt().center[0] - question_text.get_width() //  2,self.fondo.getRekt().center[1] - question_text.get_height() // 2))
