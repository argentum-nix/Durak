...
class Juego(st.Estados_Juego):
    def __init__(self, nJugadores):
        ...
        self.manoVisible = self.jugadores[0].manoAcotada()
        ...
        # self.inactive_ai = "Grey_1.png"
        # self.active_ai = "Blue_1.png"
        ...
        self.bot_ai = list(map(lambda i: BotonCarta(
            self.pos[i][0], self.pos[i][1], 70, 103, "Grey_1.png", False), [i for i in range(5)]))

        # t1 es la carta trump
        self.t1 = BotonCarta(100, 400, 50, 67, self.trump.fileNaipe(), False)
    ...
    def repartirCartas(self):  # Repartir cartas al inicio de la partida
        repartir = False
        # Reparte 1 carta para cada 1, 6 veces, con recursion
    ...
    def render(self, clock, screen, p):
        ...
        cpu4 = self.bot_ai[3].getImg()
        cpu5 = self.bot_ai[4].getImg()
        ...
        while not self.st_done:
            ...
            screen.blit(cpu4, (self.bot_ai[3].getX(), self.bot_ai[3].getY()))
            screen.blit(cpu5, (self.bot_ai[4].getX(), self.bot_ai[4].getY()))
